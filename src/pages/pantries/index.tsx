...
import { getPantries, deletePantryById } from 'apiSdk/pantries';
import { PantryInterface } from 'interfaces/pantry';

type ColumnType = ColumnDef<PantryInterface, unknown>;

interface PantryListPageProps {
...
export function PantryListPage(props: PantryListPageProps) {
  ...
  const { data, error, isLoading, mutate } = useSWR<PaginatedInterface<PantryInterface>>(
    () => `/pantries?params=${JSON.stringify(params)}`,
    fetcher,
  );
  ...
  const handleView = (row: PantryInterface) => {
    if (hasAccess('pantry', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)) {
      router.push(`/pantries/view/${row.id}`);
    }
  };
      ...
      : null,
    {
      ...
      cell: ({ row: { original: record } }: any) => (
        <Flex justifyContent="flex-end">
          <NextLink href={`/pantries/view/${record.id}`} passHref legacyBehavior>
            <Button
              onClick={(e) => e.stopPropagation()}
              mr={2}
              padding="0rem 8px"
          ...
          {hasAccess('pantry', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
            <NextLink href={`/pantries/edit/${record.id}`} passHref legacyBehavior>
              <Button
                onClick={(e) => e.stopPropagation()}
                mr={2}
                padding="0rem 0.5rem"
          ...
          {hasAccess('pantry', AccessOperationEnum.DELETE, AccessServiceEnum.PROJECT) && (
            <IconButton
              onClick={(e) => {
                e.stopPropagation();
  ...
  return (
    <Box p={4} rounded="md" shadow="none">
      <AccessInfo entity="pantry" />
      <Flex justifyContent="space-between" mb={4}>
        <Text as="h1" fontSize="1.875rem" fontWeight="bold" color="base.content" {...titleProps}>
          Pantries
        </Text>
        {hasAccess('pantry', AccessOperationEnum.CREATE, AccessServiceEnum.PROJECT) && (
          <NextLink href={`/pantries/create`} passHref legacyBehavior>
            <Button
              onClick={(e) => e.stopPropagation()}
              height={'2rem'}
